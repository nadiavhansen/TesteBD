...
class TestDataBase(TestCase):
    ...
    @mock.patch("app.database.DataBase.is_database_selected")
    @mock.patch("app.database.DataBase.conn_and_cursor_exist")
    def test_read_all_data_works(self, mock_conn_and_cursor_exist, mock_is_database_selected):
        mock_conn_and_cursor_exist.return_value = False
        with self.assertRaises(Exception) as error:
    ...
    @mock.patch("app.database.DataBase.is_database_selected")
    @mock.patch("app.database.DataBase.conn_and_cursor_exist")
    def test_read_selected_data_works(self, mock_conn_and_cursor_exist, mock_is_database_selected) :
        mock_conn_and_cursor_exist.return_value = False
        with self.assertRaises(Exception) as error:
            DataBase().read_selected_data("", "", "")
        self.assertEqual("Connetion or cursor is not defined!", error.exception.args[0])
        mock_conn_and_cursor_exist.return_value = True
    ...
    def test_convert_dict_to_sql_string_works(self):
        dict_to_test = dict(nome="aaa", idade=22, cpf="000")

        result = DataBase().convert_dict_to_sql_string(dict_to_test)
        self.assertEqual(result, "nome = 'aaa',idade = 22,cpf = '000'")
    ...
    def test_update_data_works(self, mock_conn_and_cursor_exist, mock_is_database_selected):
        ...
        with mock.patch("app.database.DataBase.cursor", create=True) as mock_cursor:
            ...
            self.assertTrue(DataBase().update_data("", {}, "", ""))
        self.assertFalse(DataBase().update_data("", {}, "", ""))
    ...
    def test_delete_data_works(self, mock_conn_and_cursor_exist, mock_is_database_selected):
        ...
        with mock.patch("app.database.DataBase.cursor", create=True) as mock_cursor:
            ...
            self.assertTrue(DataBase().delete_data("", "", ""))
        self.assertFalse(DataBase().delete_data("", "", ""))
